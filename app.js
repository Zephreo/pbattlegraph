...
function renderBattleView() {
    ...
    updateOverallMatchup();
    renderSuggestionsSection();
}

function updateOverallMatchup() {
    const overallMatchupEl = document.getElementById('overall-matchup');
    if (!overallMatchupEl) return;

    // Get battle teams (first 6 only)
    const myBattleTeam = state.myTeam.slice(0, BATTLE_TEAM_SIZE)
        .map((slot, idx) => ({ slot, idx }))
        .filter(s => s.slot.pokemon !== null && !state.faintedMyTeam[s.idx]);
    const enemyBattleTeam = state.enemyTeam.slice(0, BATTLE_TEAM_SIZE)
        .map((slot, idx) => ({ slot, idx }))
        .filter(s => s.slot.pokemon !== null);

    if (myBattleTeam.length === 0 || enemyBattleTeam.length === 0) {
        overallMatchupEl.textContent = '';
        overallMatchupEl.className = 'overall-matchup';
        return;
    }

    // Calculate average matchup score across all matchups
    let totalScore = 0;
    let matchupCount = 0;

    myBattleTeam.forEach(myEntry => {
        enemyBattleTeam.forEach(enemyEntry => {
            totalScore += calculateMatchupScore(myEntry.slot, enemyEntry.slot);
            matchupCount++;
        });
    });

    const averageScore = matchupCount > 0 ? Math.round(totalScore / matchupCount) : 50;

    // Update display
    overallMatchupEl.textContent = `${averageScore}%`;

    // Set color class based on score
    overallMatchupEl.className = 'overall-matchup';
    if (averageScore >= 55) {
        overallMatchupEl.classList.add('favorable');
    } else if (averageScore <= 45) {
        overallMatchupEl.classList.add('unfavorable');
    } else {
        overallMatchupEl.classList.add('neutral');
    }
}
...
// ============================================
// BATTLE SUGGESTIONS ANALYSIS
// ============================================

/**
 * Get all type combinations that actually exist in the game
 * Builds from POKEMON_DATA to ensure we only recommend real type combos
 */
function getExistingTypeCombos() {
    const comboSet = new Set();

    POKEMON_DATA.forEach(pokemon => {
        // Sort types alphabetically to ensure consistent keys
        const sortedTypes = [...pokemon.types].sort();
        const key = sortedTypes.join('/');
        comboSet.add(key);
    });

    // Convert back to arrays
    return Array.from(comboSet).map(key => key.split('/'));
}

// Cache the existing type combos (computed once)
let _existingTypeCombosCache = null;

/**
 * Generate all type combinations that exist in the game
 */
function generateAllTypeCombos() {
    if (!_existingTypeCombosCache) {
        _existingTypeCombosCache = getExistingTypeCombos();
    }
    return _existingTypeCombosCache;
}

/**
 * Find all Pokemon with a specific type combination
 * Returns Pokemon sorted by base stat total (highest first)
 */
function getPokemonByTypeCombo(typeCombo) {
    const sortedCombo = [...typeCombo].sort();

    return POKEMON_DATA
        .filter(pokemon => {
            const pokemonTypes = [...pokemon.types].sort();
            // Must match exactly (same types, same count)
            if (pokemonTypes.length !== sortedCombo.length) return false;
            return pokemonTypes.every((t, i) => t === sortedCombo[i]);
        })
        .map(pokemon => ({
            ...pokemon,
            bst: pokemon.hp + pokemon.attack + pokemon.defense + pokemon.spAtk + pokemon.spDef + pokemon.speed
        }))
        .sort((a, b) => b.bst - a.bst);
}

/**
 * Get top N Pokemon for a type combo
 */
function getTopPokemonForTypeCombo(typeCombo, limit = 3) {
    return getPokemonByTypeCombo(typeCombo).slice(0, limit);
}

/**
 * Analyze offensive needs - find enemies we struggle against
 * Returns map of enemy name -> { types, bestScore }
 */
function analyzeOffensiveNeeds(myBattleTeam, enemyBattleTeam) {
    const needs = [];

    enemyBattleTeam.forEach(enemyEntry => {
        const enemySlot = enemyEntry.slot;
        const enemyTypes = enemySlot.teraType ? [enemySlot.teraType] : enemySlot.pokemon.types;

        // Find our best matchup against this enemy
        let bestMatchup = 0;
        myBattleTeam.forEach(myEntry => {
            const score = calculateMatchupScore(myEntry.slot, enemySlot);
            if (score > bestMatchup) bestMatchup = score;
        });

        // If we struggle (score < 60), record this as a need
        if (bestMatchup < 60) {
            needs.push({
                name: enemySlot.pokemon.name,
                types: enemyTypes,
                bestScore: bestMatchup,
                severity: 60 - bestMatchup // Higher = we struggle more
            });
        }
    });
...
/**
 * Analyze defensive needs - find types our team is weak to
 * Returns map of type -> { count, weakPokemon }
 */
function analyzeDefensiveNeeds(myBattleTeam) {
    const weaknesses = {};

    myBattleTeam.forEach(entry => {
        const slot = entry.slot;
        const defTypes = slot.teraType ? [slot.teraType] : slot.pokemon.types;

        ALL_TYPES.forEach(attackType => {
            const effectiveness = getEffectiveness(attackType, defTypes);
            if (effectiveness >= 2) {
                if (!weaknesses[attackType]) {
                    weaknesses[attackType] = { count: 0, weakPokemon: [], totalMultiplier: 0 };
                }
                weaknesses[attackType].count++;
                weaknesses[attackType].weakPokemon.push(slot.pokemon.name);
                weaknesses[attackType].totalMultiplier += effectiveness;
            }
        });
    });

    // Filter to only types that hit 2+ Pokemon
    const significantWeaknesses = {};
    Object.entries(weaknesses).forEach(([type, data]) => {
        if (data.count >= 2) {
            significantWeaknesses[type] = data;
        }
    });
...
/**
 * Score a type combination for offensive value
 * Higher score = better coverage against enemies we struggle with
 */
function scoreTypeComboOffense(typeCombo, offensiveNeeds) {
    let score = 0;
    const hitsEnemies = [];

    offensiveNeeds.forEach(need => {
        // Check if either type in the combo hits this enemy super effectively
        typeCombo.forEach(attackType => {
            const effectiveness = getEffectiveness(attackType, need.types);
            if (effectiveness >= 2) {
                // Weight by how much we struggle and effectiveness
                score += need.severity * effectiveness;
                if (!hitsEnemies.includes(need.name)) {
                    hitsEnemies.push(need.name);
                }
            }
        });
    });

    return { score, hitsEnemies };
}

/**
 * Score a type combination for defensive value
 * Higher score = resists more types our team is weak to
 */
function scoreTypeComboDefense(typeCombo, defensiveNeeds) {
    let score = 0;
    const resistsTypes = [];

    Object.entries(defensiveNeeds).forEach(([threatType, data]) => {
        const effectiveness = getEffectiveness(threatType, typeCombo);
        if (effectiveness < 1) {
            // This combo resists or is immune to the threat
            const resistValue = effectiveness === 0 ? 2 : (1 - effectiveness); // Immunity worth more
            score += data.count * resistValue * 10;
            resistsTypes.push({
                type: threatType,
                count: data.count,
    ...
    return { score, resistsTypes };
}

/**
 * Check if a type combo has severe weaknesses we should warn about
 */
function getTypeComboWeaknesses(typeCombo) {
    const weaknesses = [];

    ALL_TYPES.forEach(attackType => {
        const effectiveness = getEffectiveness(attackType, typeCombo);
        if (effectiveness >= 4) {
            weaknesses.push({ type: attackType, multiplier: 4 });
        } else if (effectiveness >= 2) {
            weaknesses.push({ type: attackType, multiplier: 2 });
        }
    });
...
/**
 * Calculate recommended type combinations
 * Returns top combos sorted by combined offensive + defensive score
 */
function calculateRecommendedTypes(myBattleTeam, enemyBattleTeam) {
    const offensiveNeeds = analyzeOffensiveNeeds(myBattleTeam, enemyBattleTeam);
    const defensiveNeeds = analyzeDefensiveNeeds(myBattleTeam);

    // If no significant needs, return empty
    if (offensiveNeeds.length === 0 && Object.keys(defensiveNeeds).length === 0) {
        return [];
    }

    const allCombos = generateAllTypeCombos();
    const scoredCombos = [];

    allCombos.forEach(combo => {
        const offense = scoreTypeComboOffense(combo, offensiveNeeds);
        const defense = scoreTypeComboDefense(combo, defensiveNeeds);
        const weaknesses = getTypeComboWeaknesses(combo);

        // Skip combos that don't help at all
        if (offense.score === 0 && defense.score === 0) return;

        // Penalize combos with 4x weaknesses to types we're already weak to
        let penalty = 0;
        weaknesses.forEach(w => {
            if (defensiveNeeds[w.type]) {
                penalty += w.multiplier === 4 ? 20 : 5;
            }
        });

        const totalScore = offense.score + defense.score - penalty;
        ...
        if (totalScore > 0) {
            scoredCombos.push({
                ...
                totalScore,
                hitsEnemies: offense.hitsEnemies,
                resistsTypes: defense.resistsTypes,
                weaknesses: weaknesses.filter(w => w.multiplier >= 4)
            });
        }
    });
...
/**
 * Find enemy Pokemon that none of our team handles well
 */
function findUncounteredThreats(myBattleTeam, enemyBattleTeam) {
    const threats = [];
    ...
    enemyBattleTeam.forEach(enemyEntry => {
        ...
        myBattleTeam.forEach(myEntry => {
            const score = calculateMatchupScore(myEntry.slot, enemySlot);
            if (score > bestMatchup) {
                bestMatchup = score;
                bestCounter = myEntry.slot.pokemon.name;
            }
        });

        // If no one handles them well (score < 55), it's a threat
        if (bestMatchup < 55) {
            threats.push({
                pokemon: enemySlot.pokemon.name,
                types: enemySlot.teraType ? [enemySlot.teraType] : enemySlot.pokemon.types,
                bestMatchup: Math.round(bestMatchup),
                bestCounter
            });
        }
    });

    return threats.sort((a, b) => a.bestMatchup - b.bestMatchup);
}

/**
 * Find Pokemon on our team that have poor overall matchups
 */
function findWeakTeamMembers(myBattleTeam, enemyBattleTeam) {
    const weakMembers = [];

    myBattleTeam.forEach(myEntry => {
        let totalScore = 0;
        let worstMatchup = 100;
        let worstEnemy = null;

        enemyBattleTeam.forEach(enemyEntry => {
            const score = calculateMatchupScore(myEntry.slot, enemyEntry.slot);
            totalScore += score;
            if (score < worstMatchup) {
                worstMatchup = score;
                worstEnemy = enemyEntry.slot.pokemon.name;
            }
        });

        const avgScore = enemyBattleTeam.length > 0 ? totalScore / enemyBattleTeam.length : 50;

        // Flag anyone with average < 45%
        if (avgScore < 45) {
            weakMembers.push({
                pokemon: myEntry.slot.pokemon.name,
                avgScore: Math.round(avgScore),
                worstMatchup: Math.round(worstMatchup),
                worstEnemy
            });
        }
    });

    return weakMembers.sort((a, b) => a.avgScore - b.avgScore);
}

/**
 * Analyze enemy move type threats
 */
function analyzeEnemyMoveThreats(myBattleTeam, enemyBattleTeam) {
    const moveTypeThreats = {};

    enemyBattleTeam.forEach(enemyEntry => {
        const enemySlot = enemyEntry.slot;
        enemySlot.moves.forEach(moveName => {
            if (!moveName) return;
            const move = getMoveByName(moveName);
            ...
            myBattleTeam.forEach(myEntry => {
                const myTypes = myEntry.slot.teraType ? [myEntry.slot.teraType] : myEntry.slot.pokemon.types;
                const effectiveness = getEffectiveness(move.type, myTypes);
                if (effectiveness >= 2) {
                    threatenedCount++;
                    threatenedList.push(myEntry.slot.pokemon.name);
                }
            });

            if (threatenedCount >= 2) {
                if (!moveTypeThreats[move.type]) {
                    moveTypeThreats[move.type] = {
                        users: [],
                        threatened: new Set(),
                        count: 0
                    };
                }
                moveTypeThreats[move.type].users.push(enemySlot.pokemon.name);
                threatenedList.forEach(p => moveTypeThreats[move.type].threatened.add(p));
                moveTypeThreats[move.type].count = moveTypeThreats[move.type].threatened.size;
            }
        });
    });

    return Object.entries(moveTypeThreats)
        .map(([type, data]) => ({
            type,
...
/**
 * Generate all battle suggestions
 */
function generateBattleSuggestions() {
    // Get battle teams (first 6 only, excluding fainted)
    const myBattleTeam = state.myTeam.slice(0, BATTLE_TEAM_SIZE)
        .map((slot, idx) => ({ slot, idx }))
        .filter(s => s.slot.pokemon !== null && !state.faintedMyTeam[s.idx]);
    ...
    return {
        recommendedTypes: calculateRecommendedTypes(myBattleTeam, enemyBattleTeam),
        uncounteredThreats: findUncounteredThreats(myBattleTeam, enemyBattleTeam),
        weakMembers: findWeakTeamMembers(myBattleTeam, enemyBattleTeam),
        moveTypeThreats: analyzeEnemyMoveThreats(myBattleTeam, enemyBattleTeam)
    };
}
...
function renderSuggestionsSection() {
    ...
    const suggestions = generateBattleSuggestions();

    if (!suggestions) {
        container.innerHTML = `
            <div class="no-suggestions">
                <div class="no-suggestions-icon">💭</div>
                <p>Add Pokemon to both teams to see analysis</p>
            </div>
        `;
        return;
    }

    const { recommendedTypes, uncounteredThreats, weakMembers, moveTypeThreats } = suggestions;

    // Generate star rating based on score
    function getStarRating(score, maxScore) {
        if (maxScore === 0) return '';
        const normalized = Math.min(score / maxScore, 1);
        const stars = Math.ceil(normalized * 4);
        return '★'.repeat(stars) + '☆'.repeat(4 - stars);
    }

    // Get max score for normalization
    const maxScore = recommendedTypes.length > 0 ? recommendedTypes[0].totalScore : 1;

    // Build HTML
    let html = '<div class="suggestions-grid">';

    // Recommended Types Card
    html += `
        <div class="suggestion-card recommended-types">
            <div class="suggestion-card-header">
                <span class="suggestion-icon">🎯</span>
                <h4>Recommended Types to Find</h4>
            </div>
            <div class="suggestion-card-body">
    `;

    if (recommendedTypes.length > 0) {
        html += '<div class="type-recommendations">';
        recommendedTypes.forEach(rec => {
            const typeBadges = rec.types.map(t =>
                `<span class="type-badge type-${t}">${t}</span>`
            ).join('');

            const hitsText = rec.hitsEnemies.length > 0
                ? `<span class="reason-label">Hits:</span> <span class="reason-hits">${rec.hitsEnemies.slice(0, 2).join(', ')}</span>`
                : '';

            const resistsText = rec.resistsTypes.length > 0
                ? `<span class="reason-label">Resists:</span> <span class="reason-resists">${rec.resistsTypes.slice(0, 2).map(r => r.type).join(', ')}</span>`
                : '';

            const reasonParts = [hitsText, resistsText].filter(Boolean);

            // Get top 3 Pokemon for tooltip
            const topPokemon = getTopPokemonForTypeCombo(rec.types, 3);
            const tooltipText = topPokemon.length > 0
                ? `Top: ${topPokemon.map(p => `${p.name} (${p.bst})`).join(', ')}`
                : 'No Pokemon found';

            html += `
                <div class="type-recommendation"
                     data-types="${rec.types.join(',')}"
                     title="${tooltipText}">
                    <div class="type-recommendation-header">
                        <div class="type-badges">${typeBadges}</div>
                        <span class="star-rating">${getStarRating(rec.totalScore, maxScore)}</span>
                    </div>
                    ${reasonParts.length > 0 ? `<div class="reason">${reasonParts.join(' | ')}</div>` : ''}
                    <div class="pokemon-preview">
                        ${topPokemon.slice(0, 3).map(p => `<span class="preview-pokemon">${p.name}</span>`).join('')}
                    </div>
                </div>
            `;
        });
        ...
        html += '<p class="suggestion-description" style="color: var(--accent-secondary);">Good coverage! No major gaps detected.</p>';
    }
    ...
    // Add uncountered threats
    uncounteredThreats.slice(0, 2).forEach(threat => {
        const typeStr = threat.types.map(t =>
            `<span class="type-badge type-${t}" style="font-size:0.65rem;padding:0.05rem 0.3rem;">${t}</span>`
        ).join(' ');
        insights.push({
            type: 'warning',
            icon: '⚠️',
            text: `No strong counter for <strong>${threat.pokemon}</strong> ${typeStr} — best is ${threat.bestCounter} at ${threat.bestMatchup}%`
        });
    });

    // Add weak team member warnings
    weakMembers.slice(0, 1).forEach(member => {
        insights.push({
            type: 'warning',
            icon: '📉',
            text: `<strong>${member.pokemon}</strong> struggles against this team (${member.avgScore}% avg) — consider swapping`
        });
    });

    // Add move type threats
    moveTypeThreats.slice(0, 1).forEach(threat => {
        const userStr = threat.users.slice(0, 2).join(', ');
        insights.push({
            type: 'tip',
            icon: '💥',
            text: `<span class="type-badge type-${threat.type}" style="font-size:0.65rem;padding:0.05rem 0.3rem;">${threat.type}</span> moves from ${userStr} threaten ${threat.count} of your Pokemon`
        });
    });

    // If team is doing well
    if (insights.length === 0) {
        insights.push({
            type: 'info',
            icon: '✅',
            text: 'Your team composition looks solid against this opponent!'
        });
    }

    insights.forEach(insight => {
        html += `
            <li class="suggestion-item ${insight.type}">
                <span class="suggestion-item-icon">${insight.icon}</span>
    ...
    html += '</ul></div></div>';
    html += '</div>'; // Close suggestions-grid

    // Add expanded Pokemon list container
    html += '<div id="expanded-pokemon-list" class="expanded-pokemon-list" style="display: none;"></div>';

    container.innerHTML = html;

    // Add click handlers for type recommendations
    container.querySelectorAll('.type-recommendation[data-types]').forEach(el => {
        el.style.cursor = 'pointer';
        el.addEventListener('click', () => {
            const types = el.dataset.types.split(',');
            showExpandedPokemonList(types);
        });
    });
}

/**
 * Show expanded list of all Pokemon with a type combination
 */
function showExpandedPokemonList(types) {
    const container = document.getElementById('expanded-pokemon-list');
    if (!container) return;

    const pokemon = getPokemonByTypeCombo(types);

    if (container.style.display === 'block' && container.dataset.types === types.join(',')) {
    ...
    container.dataset.types = types.join(',');

    let html = `
        <div class="expanded-header">
            <div class="expanded-title">
                ${types.map(t => `<span class="type-badge type-${t}">${t}</span>`).join('')}
                <span class="expanded-count">${pokemon.length} Pokemon</span>
            </div>
            <button class="expanded-close" onclick="document.getElementById('expanded-pokemon-list').style.display='none'">✕</button>
        </div>
        <div class="expanded-pokemon-grid">
    `;

    pokemon.forEach(p => {
        const spriteUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${p.id}.png`;
        html += `
            <div class="expanded-pokemon-item">
                <img src="${spriteUrl}" alt="${p.name}" loading="lazy">
                <div class="expanded-pokemon-info">
                    <span class="expanded-pokemon-name">${p.name}</span>
                    <span class="expanded-pokemon-bst">BST: ${p.bst}</span>
                </div>
            </div>
        `;
    });
...
function setupEventListeners() {
    ...
    // Suggestions section collapse toggle
    const collapseSuggestionsBtn = document.getElementById('collapse-suggestions');
    const suggestionsSection = document.getElementById('battle-suggestions');
    if (collapseSuggestionsBtn && suggestionsSection) {
        // Restore collapsed state from localStorage
        const isCollapsed = localStorage.getItem('suggestionsSectionCollapsed') === 'true';
        if (isCollapsed) {
            suggestionsSection.classList.add('collapsed');
        }

        collapseSuggestionsBtn.addEventListener('click', () => {
            suggestionsSection.classList.toggle('collapsed');
            localStorage.setItem('suggestionsSectionCollapsed',
